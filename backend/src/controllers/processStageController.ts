...
const generateStageCode = async (templateId: number): Promise<string> => {
  // 获取当前所有的stage_code，以此计算最大值，避免因删除导致的重复
  const [rows] = await pool.execute(
    'SELECT stage_code FROM process_stages WHERE template_id = ?',
    [templateId]
  ) as any;
  ...
  if (Array.isArray(rows)) {
    for (const row of rows) {
      ...
      // 检查格式是否为 PS-数字
      if (typeof code === 'string' && code.startsWith('PS-')) {
        const suffixStr = code.substring(3);
  ...
  return `PS-${nextSuffix.toString().padStart(3, '0')}`;
};
