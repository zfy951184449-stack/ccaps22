/**
 * Scheduling V5 - Solve Result Handler
 *
...
import { progressEmitterV5, EnrichedAssignment, ResultSummaryV5 } from './types';
import {
    parseRunSummary,
...
export const getSolveResultV5 = async (req: Request, res: Response) => {
    try {
        const { runId } = req.params;

        const [rows] = await pool.execute<RowDataPacket[]>(
            'SELECT result_summary, summary_json, target_batch_ids, window_start, window_end, solver_progress FROM scheduling_runs WHERE id = ?',
            [runId]
        );
        ...
        // V5 附加字段（§1.3 / §1.4 / §1.5）。
        // infeasibility_analysis：从 result 顶层读（solver 写入，saveResults 持久化在 result_summary）。
        const infeasibilityAnalysis = extractInfeasibilityAnalysis(rawResult);
        // objective_breakdown：冻结路径 result.metrics.objective_breakdown（§1.4）。
        const objectiveBreakdown = rawResult?.metrics?.objective_breakdown ?? null;
        // viz：从 solver_progress 读取 V5 累积结构（convergence/events/infeasibility/phase/viz_meta 等）。
        const solverProgressRaw = run.solver_progress;
        let vizData: any = null;
        if (solverProgressRaw != null) {
            try {
                const sp = typeof solverProgressRaw === 'string'
                    ? JSON.parse(solverProgressRaw)
                    : solverProgressRaw;
                // viz 包含所有 V5 累积 viz 键（convergence/events/phase/model_stats/search_stats/infeasibility/viz_meta）。
                vizData = {
                    phase: sp?.phase ?? null,
                    model_stats: sp?.model_stats ?? null,
                    search_stats: sp?.search_stats ?? null,
                    convergence: sp?.convergence ?? [],
                    events: sp?.events ?? [],
                    infeasibility: sp?.infeasibility ?? null,
                    viz_meta: sp?.viz_meta ?? { convergence_count: 0, events_count: 0 },
                };
            } catch {
                vizData = null;
            }
        ...
        if (empIds.length > 0) {
            try {
                ...
                console.warn('[SchedulingV5] employee_unavailability 读取失败,跳过不可用过滤:', e?.message);
            }
        }
        ...
        console.error('[SchedulingV5] Get Result Failed:', error);
        res.status(500).json({ error: error.message });
    }
}

export const receiveSolveResultV5 = async (req: Request, res: Response) => {
    try {
        ...
        console.log(`[SchedulingV5] Received final result via callback for Run ${run_id}`);
        ...
        if (existing.length > 0 && existing[0].status === 'APPLIED') {
            console.log(`[SchedulingV5] Run ${run_id} already APPLIED, ignoring late result callback.`);
        ...
        console.log(`[SchedulingV5] Run ${run_id} updated to ${finalStatus} via callback`);

        // 读取最新 solver_progress（含 V5 viz 累积结构），一并透传给 SSE 客户端。
        let finalSolverProgress: any = null;
        try {
            const [spRows] = await pool.execute<RowDataPacket[]>(
                'SELECT solver_progress FROM scheduling_runs WHERE id = ?',
                [run_id]
            );
            if (spRows.length > 0 && spRows[0].solver_progress != null) {
                finalSolverProgress = spRows[0].solver_progress;
            }
        } catch (spErr) {
            console.warn(`[SchedulingV5] Failed to read solver_progress for Run ${run_id}:`, spErr);
        }
        ...
        progressEmitterV5.emit(`run:${run_id}`, {
            ...
            stage: 'DONE',
            error: errorMsg,
            solver_progress: finalSolverProgress
        });

        res.json({ success: true });

    } catch (error: any) {
        console.error('[SchedulingV5] Receive Result Failed:', error);
        res.status(500).json({ error: error.message });
    }
};
