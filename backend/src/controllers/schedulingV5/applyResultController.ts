/**
 * Scheduling V5 - Apply Result Controller
 *
...
export const applySolveResultV5 = async (req: Request, res: Response) => {
    ...
    try {
        ...
        console.log(`[SchedulingV5] Applying result for Run ${runId}...`);
        ...
        if (hasOverride) {
            ...
            // assignments / shift_schedule 这条展平分支。
            rawResult = {
                ...rawResult,
                schedules: undefined,
            ...
            console.log(`[SchedulingV5] Run ${runId} 手动覆盖应用: ${rawResult.assignments.length} 条分配, ${rawResult.shift_schedule.length} 条班次 (force=${forceReapply}, prevStatus=${run.status})`);
        }
        ...
        console.log(`[SchedulingV5][L1] Run ${runId} scope: fullyGlobal=${isFullyGlobal}, teams=${scopeTeamIds.length}, batches=${scopeBatchIds.length}, emps=${scopeEmployeeIds.length}, standalone=${scopeStandaloneTaskIds.length}`);
        ...
        const totalTasksInSchedules = schedulesPresent
            ...
            : 0;
        console.log(`[SchedulingV5][Diag] Result keys: [${resultKeys.join(', ')}]`);
        console.log(`[SchedulingV5][Diag] schedules present: ${schedulesPresent}, count: ${schedulesLength}, total tasks: ${totalTasksInSchedules}`);
        console.log(`[SchedulingV5][Diag] assignments present: ${Array.isArray(rawResult.assignments)}, shift_schedule present: ${Array.isArray(rawResult.shift_schedule)}`);
        ...
        if (Array.isArray(rawResult.schedules) && rawResult.schedules.length > 0) {
            ...
            console.log(`[SchedulingV5] 展平 ${rawResult.schedules.length} 条 schedules → ${assignments.length} 条人员分配, ${shiftSchedule.length} 条班次计划`);
        } else {
        ...
        if (isIntervalSolve) {
            console.log(`[SchedulingV5] Interval solve mode: cleanup range ${cleanupStart} ~ ${cleanupEnd} (full window ${windowStart} ~ ${windowEnd})`);
        }
        ...
        if (cleanupStart && cleanupEnd) {
            ...
            if (scopeEmployeeIds.length > 0) {
                ...
                console.warn(`[SchedulingV5][L1] Run ${runId} 局部求解但无员工责任域键(未选团队),跳过 esp 清理(避免误删别团队)`);
            }
            ...
            if (scopeBatchIds.length > 0) {
                ...
                console.log(`[SchedulingV5] Cleaned up non-locked assignments in ${isIntervalSolve ? 'solve range' : 'window'} ${cleanupStart} ~ ${cleanupEnd} (batches ${scopeBatchIds.join(',')})`);
            } else if (isFullyGlobal) {
                await connection.execute(
                    `DELETE bpa FROM batch_personnel_assignments bpa
                     JOIN batch_operation_plans bop ON bpa.batch_operation_plan_id = bop.id
                ...
                console.log(`[SchedulingV5] Cleaned up non-locked assignments in ${isIntervalSolve ? 'solve range' : 'window'} ${cleanupStart} ~ ${cleanupEnd}`);
            } else {
                console.warn(`[SchedulingV5][L1] Run ${runId} 局部求解但无批次责任域键,跳过 bpa 清理(避免误删别团队)`);
            }
            ...
            if (scopeStandaloneTaskIds.length > 0) {
                ...
                console.warn(`[SchedulingV5][L1] Run ${runId} 局部求解但无独立任务责任域键,跳过 standalone 清理(避免误删别团队)`);
            }
        }
        ...
        for (const assignment of assignments) {
            try {
                if (assignment.is_standalone) {
                    ...
                    if (lockedAssignmentKeys.has(assignmentKey)) {
                        ...
                        if (lockedEmployeeId !== assignment.employee_id) {
                            console.warn(
                                `[SchedulingV5] Skip locked assignment conflict: Op ${assignment.operation_id} Pos ${assignment.position_number} locked to Emp ${lockedEmployeeId}, solver returned Emp ${assignment.employee_id}`
                            );
                        }
                ...
                console.warn(`[SchedulingV5] Failed to insert assignment: Op ${assignment.operation_id} Pos ${assignment.position_number} -> Emp ${assignment.employee_id}: ${err.message}`);
            }
        }
            ...
            try {
                ...
                if (lockedShiftPlan) {
                    if (lockedShiftPlan.shift_id !== plan.shift_id) {
                        console.warn(
                            `[SchedulingV5] Skip locked shift conflict: Emp ${plan.employee_id} ${plan.date} locked to Shift ${lockedShiftPlan.shift_id}, solver returned Shift ${plan.shift_id}`
                        );
                ...
                console.warn(`[SchedulingV5] Failed to insert shift plan: Emp ${plan.employee_id} ${plan.date} -> Shift ${plan.shift_id}: ${err.message}`);
            }
        }
        ...
        if (shiftLinkBackfill.updatedAssignments > 0 || shiftLinkBackfill.ambiguousAssignments > 0 || shiftLinkBackfill.missingAssignments > 0) {
            console.log(
                `[SchedulingV5] Reconciled shift links for Run ${runId}: ` +
        ...
        if (hasOverride) {
            ...
            // (否则 getSolveResultV5 仍回读求解器原始结果,手改在 UI 上会"丢失")。
            await connection.execute(
        ...
        console.log(
            `[SchedulingV5] Applied result for Run ${runId}: ${assignmentsInserted} batch assignments, ` +
        ...
        console.error('[SchedulingV5] Apply Result Failed:', error);
