/**
 * Scheduling V5 - Precheck Handler
 *
 * Assembles solver request data and proxies it to the Solver /api/v5/precheck endpoint.
 * Frontend calls POST /api/v5/scheduling/precheck with { batch_ids, start_date, end_date, config }.
 */
import { Request, Response } from 'express';
import { DataAssemblerV4 } from '../../services/schedulingV4/DataAssemblerV4';
import { SOLVER_V5_URL } from './types';

export const runPrecheckV5 = async (req: Request, res: Response) => {
    try {
        ...
        console.log(`[SchedulingV5] Precheck requested: ${start_date} ~ ${end_date}, ${batch_ids.length} batches`);
        ...
        console.log(`[SchedulingV5] Precheck data assembled: ${solverRequest.operation_demands.length} ops, ${solverRequest.employee_profiles.length} employees`);
        ...
        const response = await fetch(`${SOLVER_V5_URL}/api/v5/precheck`, {
            method: 'POST',
        ...
        if (!response.ok) {
            ...
            console.error(`[SchedulingV5] Solver precheck error: ${response.statusText}`, errorText);
        ...
        console.log(`[SchedulingV5] Precheck result: ${result.status} (${result.total_checks} checks)`);
        ...
        console.error('[SchedulingV5] Precheck Failed:', error);
