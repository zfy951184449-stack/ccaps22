/**
 * Scheduling V5 Preview Proposal Controller
 *
...
export const createPreviewProposalV5 = async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            return res.json({
                ...
                preview_only: true,
                capability_gap: {
                    code: 'SOLVER_V5_PREVIEW_FAILED',
                    message: `Solver V5 preview failed: ${response.status} ${response.statusText}`,
                    detail: errorText,
                },
            });
        }
        ...
        return res.json({
            ...
            preview_only: true,
            data: {
                mode: 'solver_v5_preview_adapter',
        ...
        if (error?.name === 'AbortError') {
            return res.json({
                ...
                preview_only: true,
                capability_gap: {
                    code: 'SOLVER_V5_PREVIEW_TIMEOUT',
                    message: 'Solver V5 preview timed out before returning a proposal.',
                },
            });
        }
        ...
        return res.json({
            ...
            preview_only: true,
            capability_gap: {
                code: solverUnavailable ? 'SOLVER_V5_PREVIEW_UNAVAILABLE' : 'PREVIEW_DATA_ASSEMBLY_GAP',
                message: solverUnavailable
                    ? 'Solver V5 preview service is unavailable for this preview request.'
