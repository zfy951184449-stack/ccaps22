...
class SolverCallback(cp_model.CpSolverSolutionCallback):
    ...
    def __init__(self, improvement_timeout: float = 30.0, progress_callback=None, time_limit: float = 60.0):
        super().__init__()
...
class Solver:
    ...
    def solve(self, request: SolverRequest, progress_callback=None) -> SolverResponse:
        ...
        try:
            ...
            # 1.5 冲突检测（不影响求解，只生成报告）
            logger.info("[Solver] 运行约束冲突检测...")
            conflict_detector = ConflictDetector(self.context)
