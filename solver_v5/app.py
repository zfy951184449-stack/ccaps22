...
@app.route("/api/v5/precheck", methods=["POST"])
...
ACTIVE_CALLBACKS = {}
# Lock protecting all 4 access points of ACTIVE_CALLBACKS (multi-thread gunicorn safety)
_callback_lock = threading.Lock()

@app.route("/api/v5/abort/<request_id>", methods=["POST"])
def abort_run(request_id):
    ...
    try:
        ...
        if callback:
             ...
             logger.warning(f"No active callback found for Run {request_id} to abort. Active: {list(ACTIVE_CALLBACKS.keys())}")
...
@app.route("/api/v5/solve", methods=["POST"])
def solve_endpoint():
    """
    V4 Solve Endpoint.
    ...
    try:
        ...
        try:
            ...
            with _callback_lock:
                ACTIVE_CALLBACKS.pop(request_id, None)
...
def main():
    port = int(os.environ.get("PORT", os.environ.get("SOLVER_V5_PORT", 5006)))
    debug = os.environ.get("FLASK_DEBUG", "0") == "1"
    logger.info(f"Starting Solver V5 on port {port}")
