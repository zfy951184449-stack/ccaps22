---
description: 启动 Vibe Coding 多智能体闭环工作流
---

# 标准作业程序高度透明闭环 (Workflow SOP Loop)

为了实现彻底的自我驱动和“零人工管理”（Vibe Coding），任何一次功能的下界迭代或基础问题的抛出，都必须必须严格且完整地经历以下 8 层闭环。各个节点的输出都要求**高度且直接通过聊天窗口暴露给用户，保持全白盒透明模式：**
...
- **动作：** 
  1. **Challenger 率先抛出刁难：** 强制输出至少一条逆向洞察或激进重构建议。
  2. **Planner 作答与文档撰写：** Planner 拿定底座框架，输出极度详尽的实施计划，并且**必须在出具的主计划中单开一节，明确书面回复是否采纳 Challenger 的建议及核心理由**。同时，Planner 需在此阶段亲自动手完成所有非代码类相关文档的编撰和完善落地，不能推给下游。
...
### 阶段 6: 动态交互验证 (UX Testing)
- **主导角色：** 🤖 自动化测试工程师 (Test Engineer) 
- **动作：** 项目服务器启动并准备验证。Test Engineer 开始运用 **严禁使用其余组件、有且只能用 Playwright 库** 编写端到端界面的自动化挂机游离测试程序。强行推演整个交互点击跳转，以排除产品交付后的断链、阻塞或是操作别扭。
- **内循环自驱：** 体验不行或报错卡死，开启 **第二重端到端内部循环重试 (上限 3-5 次)**。

### 阶段 7: 版本库清理与封口 (Archiving)
- **主导角色：** 🧹 清洁工 (Cleaner)
- **动作：** 所有测试彻底变绿后。Cleaner 大扫除，铲平在 QA 和 Test Engineer 试跑阶段顺手创建的脱节文件、无意义垃圾测试包及废弃残留信息。随后进行收官的 Git Add 打包与精准精炼的代码级 Commit 动作。

### 阶段 8: 使命交付阶段性汇报 (Delivery)
- **主导角色：** 🎙️ 主持人 (Host)
