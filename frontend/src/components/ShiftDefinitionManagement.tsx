...
const ShiftDefinitionManagement: React.FC = () => {
  ...
  const openEditModal = useCallback((record: ShiftDefinition) => {
    setEditing(record);
    form.setFieldsValue({
  ...
  const handleDeactivate = useCallback(async (record: ShiftDefinition) => {
    if (!record.id) return;
    try {
  ...
  const handleToggleActive = useCallback(async (record: ShiftDefinition, value: boolean) => {
    try {
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        render: (_: any, record: ShiftDefinition) => (
          <span className="wxb-table-inline-list">
            ...
            actions={[
              ...
              record.is_active
                ? {
                    ...
                    confirm: {
                      title: '确认停用',
                      description: `确认要停用班次【${record.shift_name}】吗？`,
                      okText: '确认停用',
                    },
                  }
                : {
    ...
    [handleDeactivate, handleToggleActive, openEditModal],
  );
    ...
    <div className="dashboard-page" style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      <WxbCard>
        <WxbDataTable
          ...
          columns={columns}
          dataSource={data}
          emptyState={{ description: includeInactive ? '暂无班次定义' : '暂无启用班次定义' }}
      ...
      >
        ...
        >
          ...
          <Space size="large">
            ...
            >
              <WxbTimePicker format="HH:mm" minuteStep={15} />
            </Form.Item>
            <Form.Item
              name="end_time"
              label="结束时间"
          ...
          <Form.Item name="description" label="说明">
            <WxbTextarea rows={3} placeholder="可填写班次适用范围、注意事项等" />
          </Form.Item>
