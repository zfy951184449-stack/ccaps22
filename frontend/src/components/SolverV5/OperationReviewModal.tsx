...
const OperationReviewModal: React.FC<OperationReviewModalProps> = ({ visible, onCancel, batchIds, month, onSuccess, solverConfig = DEFAULT_SOLVER_CONFIG_V5 }) => {
    ...
    const columns: ColumnsType<OperationOperation> = [
        ...
        },
        {
            ...
            width: 200,
            render: (_, record) => (
                <div className="solver-v5-time-cell">
        ...
        },
        {
            ...
            render: (_, record) => {
                if (!record.positions || record.positions.length === 0) {
                    return <span className="solver-v5-muted-text">无特殊要求</span>;
                }

                return (
                    <div className="solver-v5-position-tags">
                        {record.positions.map(pos => {
                            ...
                            return (
                                <WxbTag
                                    ...
                                    color={color}
                                    className="solver-v5-clickable-tag"
    ...
    return (
        <>
            <WxbModal
                ...
                onCancel={onCancel}
                width={900}
                className="solver-v5-review-modal"
                footer={(
            ...
            >
                ...
                {precheckResults && (
                    <div className={`solver-v5-precheck-summary solver-v5-precheck-${precheckResults.status.toLowerCase()}`}>
                        <strong>
                ...
                <div className="solver-v5-review-stats">
                ...
                <div className="solver-v5-info-panel">
                    ...
                    <div>
                        <strong>本次 V5 排班会保留已锁定的操作人员和班次</strong>
                        <p>求解器会把锁定数据当作硬约束，应用结果时也不会覆盖这些人工锁定记录。</p>
                    </div>
                </div>

                {loading && data.length === 0 ? (
                    <div className="solver-v5-loading-text">加载中...</div>
                ) : (
                    <div className="solver-v5-review-groups">
                        {/* 1. Share Groups */}
                        {Object.entries(groupedData.groups).map(([key, groupData]) => (
                            <div key={key} className="solver-v5-review-group">
                                <div className="solver-v5-review-group-title">
                                    共享组：{groupData.name}
                                </div>
                        ...
                        {groupedData.independent.length > 0 && (
                            <div className="solver-v5-review-group">
                                <div className="solver-v5-review-group-title">
                                    独立操作
                                </div>
                                <WxbDataTable<OperationOperation>
