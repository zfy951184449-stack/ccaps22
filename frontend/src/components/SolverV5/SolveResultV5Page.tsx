...
import InfeasibilityPanel from './monitor/InfeasibilityPanel';
import type { InfeasibilityGroup } from './monitor/monitorTypes';

import './SolverV5.css';

interface SolveResultV5PageProps {
    visible: boolean;
    runId: number | null;
    onClose: () => void;
}
...
interface ResultData {
    ...
    /** 结果路径无解分析（§1.5 冻结契约），仅 INFEASIBLE 时后端下发 */
    infeasibility_analysis?: {
        is_infeasible: boolean;
        located: boolean;
        diagnosed_at?: string;
        minimal_conflict_groups: InfeasibilityGroup[];
    };
}
...
const SolveResultV5Page: React.FC<SolveResultV5PageProps> = ({ visible, runId, onClose }) => {
    ...
    const fetchResult = useCallback(async () => {
        ...
        setError(null);
        try {
            const response = await fetch(`/api/v5/scheduling/runs/${runId}/result`);
    ...
    const fetchRunStatus = useCallback(async () => {
        if (!runId) return;
        try {
            const response = await fetch(`/api/v5/scheduling/runs/${runId}/status`);
            const json = await response.json();
    ...
    // 结果路径无解检测（§7 设计：infeasibility_analysis 存在且 is_infeasible=true）
    const infeasibilityAnalysis = data?.infeasibility_analysis;
    const isInfeasibleResult =
        (runStatus === 'INFEASIBLE' || infeasibilityAnalysis?.is_infeasible === true) &&
        !!infeasibilityAnalysis;

    // 结果路径 → 实时路径格式适配器（minimal_conflict_groups → groups，字段集相同）
    const infeasibilityGroups: InfeasibilityGroup[] | null = infeasibilityAnalysis
        ? (infeasibilityAnalysis.minimal_conflict_groups ?? null)
        : null;
    ...
    const handleApplyResult = async () => {
        ...
        setApplying(true);
        try {
            const res = await fetch(`/api/v5/scheduling/runs/${runId}/apply`, { method: 'POST' });
    ...
    return (
        ...
        >
            <div className="v4-result-container">
                ...
                ) : data && isInfeasibleResult ? (
                    /* ── 无解结果路径：顶替主区渲染 InfeasibilityPanel（§7 设计） ── */
                    <div className="v4-result-container" style={{ padding: '24px' }}>
                        <InfeasibilityPanel
                            groups={infeasibilityGroups}
                            located={infeasibilityAnalysis?.located}
                        />
                    </div>
                ) : data && kpiMetrics ? (
                    <>
...
export default SolveResultV5Page;
