...
[ -x "${GUNICORN_BIN}" ]                           || { log_block "缺 solver venv/gunicorn"; miss=1; }
[ "$miss" -eq 0 ] || { echo; log_block "产物不全,终止。"; exit 1; }
log_pass "产物齐全。"

# 4) 准备目录 + 可执行 ──────────────────────────────────────────
mkdir -p "${LOG_DIR}" "${LAUNCH_AGENTS_DIR}"
chmod +x "${DEPLOY_DIR}"/run-backend.sh "${DEPLOY_DIR}"/run-solver.sh
...
load_agent "${LAUNCH_AGENTS_DIR}/${SOLVER_LABEL}.plist"     "${SOLVER_LABEL}"     && log_pass "solver 已加载"
